...
@router.post("/validate/csv/emotibit")
async def validate_emotibit_csv(file: UploadFile) -> dict[str, Any]:
...
@router.post("/validate/csv/polar")
async def validate_polar_csv(file: UploadFile) -> dict[str, Any]:
...
@router.post("/validate/csv/markers")
async def validate_markers_csv(file: UploadFile) -> dict[str, Any]:
    ...
    unknown_codes = [c for c in codes_present if c not in KNOWN_CODES]
    ...
    return {
        ...
        "sessions": sorted(set(df["session_id"].astype(str).tolist())),
    }
